...
namespace SDM.Controllers
{
    public class VisureController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            {
                _logger.LogWrite("VisureController", null, ex);
        ...
        public ActionResult SavePratica(Pratica pratica)
        {
            try
            {
                ...
                if (_help.ControlloCampiNoCategoria(pratica))
                {
                    ...
                    if (_help.PraticaVisure(pratica, "save"))
                    {
                        TempData["erroreInserimentoPatronatoHome"] = "Pratica salvata correttamente";
                        return RedirectToAction("Index", "Visure");
                    }
                }

                TempData["erroreInserimentoPatronato"] = "true";
                return RedirectToAction("AggiungiPratica", "Visure");
            }
            catch (Exception ex)
            {
                _logger.LogWrite("VisureController", null, ex);
        ...
        public ActionResult EditPratica(Pratica pratica)
        {
            try
            {
                if (_help.ControlloCampiNoCategoria(pratica))
                {
                    ...
                    if (_help.PraticaVisure(pratica, "update"))
                    {
                ...
                TempData["erroreInserimentoPatronato"] = "true";
                return RedirectToAction("ModificaPratica", "Visure", new { idPratica = pratica.Id });
            }
            catch (Exception ex)
            {
                _logger.LogWrite("VisureController", null, ex);
        ...
        public ActionResult DeletePratica(int idPratica)
        {
            try
            {
                ...
                PraticaIndex model = new PraticaIndex();
                if (_help.DeleteVisure(idPratica))
                {
                    model.Pratiche = _help.PraticaVisure(Convert.ToInt32(Session["idsede"].ToString()), Session["role"].ToString(), "getall", null);
        ...
        public ActionResult SearchPratica(Pratica pratica)
        {
            ...
            {
                pratica.IdSede = Convert.ToInt32(Session["idsede"].ToString());
                List<Pratica> result = _help.PraticaVisure(0, Session["role"].ToString(), "search", pratica);
            ...
            try
            {
                ...
                if (Session["username"] != null && Session["password"] != null)
                {
                    if (authentication.Login(Session["username"].ToString(), Session["password"].ToString(), Session["role"].ToString(), true))
                    {
                        ...
                        List<Attachment> model = _help.AttachmentsVisure(idPratica, "getall");
                        return View(model);
                    }
                    else { return RedirectToAction("ErrorAuth", "Home"); }
                }
        ...
        public ActionResult UploadFile(int id, List<HttpPostedFileBase> files)
        {
            try
            {
                ...
                if (Request.Files != null && Request.Files.Count > 0 && Request.Files.Count <= 4)
                {
                    ...
                    if (ListFile.Count > 0)
                    {
                        if (!_help.AttachmentsVisure(ListFile, "upload"))
                        {
                ...
                List<Attachment> model = _help.AttachmentsVisure(id, "getall");
        ...
        public ActionResult DownloadFile(int idFile)
        {
            try
            {
                Attachment loadFile = _help.AttachmentsVisure("download", idFile);
        ...
        public ActionResult DeleateFile(int idPratica, int idFile)
        {
            try
            {
                ...
                if (_help.DeleteFileVisure(idFile))
                {
                    model = _help.AttachmentsVisure(idPratica, "getall");
                    return PartialView("TableAllegati", model);
                }
        ...
        public ActionResult DownloadExcel()
        {
            ...
            string fileName = $"Pratiche_Visure_{DateTime.Now:yyyyMMddHHmmss}.xlsx";
            ...
            try
            {
                List<Pratica> pratiche = _help.PraticaVisure(Convert.ToInt32(Session["idsede"].ToString()), Session["role"].ToString(), "getall", null);
