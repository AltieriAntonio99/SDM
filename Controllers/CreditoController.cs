    ...
    public class CreditoController : Controller
    {
            ...
            try
            {
                ...
                if (Session["username"] != null && Session["password"] != null)
                {
                    if (authentication.Login(Session["username"].ToString(), Session["password"].ToString(), Session["role"].ToString()))
                    {
                        ...
                        {
                            Pratiche = _help.GetPraticheCredito(Convert.ToInt32(Session["idsede"].ToString()), Session["role"].ToString()),
                            Categorie = _help.GetCategorie("credito")
                        };
            ...
            try
            {
                ...
                if (Session["username"] != null && Session["password"] != null)
                {
                    if (authentication.Login(Session["username"].ToString(), Session["password"].ToString(), Session["role"].ToString()))
                    {
                        ...
                        Pratica model = _help.GetPraticaCredito(idPratica);
        ...
        public ActionResult SavePratica(Pratica pratica)
        {
            try
            {
                ...
                if (_help.ControlloCampi(pratica))
                {
                    ...
                    if (_help.SalvaPraticaCredito(pratica))
                    {
        ...
        public ActionResult EditPratica(Pratica pratica)
        {
            try
            {
                if (_help.ControlloCampi(pratica))
                {
                    ...
                    if (_help.ModificaPraticaCredito(pratica))
                    {
        ...
        public ActionResult DeletePratica(int idPratica)
        {
            try
            {
                ...
                PraticaIndex model = new PraticaIndex();
                if (_help.DelateCredito(idPratica))
                {
                    model.Pratiche = _help.GetPraticheCredito(Convert.ToInt32(Session["idsede"].ToString()), Session["role"].ToString());
        ...
        public ActionResult SearchPratica(Pratica pratica)
        {
            ...
            {
                pratica.IdSede = Convert.ToInt32(Session["idsede"].ToString());
                List<Pratica> result = _help.RicercaCredito(pratica, Session["role"].ToString());
            ...
            try
            {
                ...
                if (Session["username"] != null && Session["password"] != null)
                {
                    if (authentication.Login(Session["username"].ToString(), Session["password"].ToString(), Session["role"].ToString()))
                    {
                        ...
                        List<Attachment> model = _help.GetFileCredito(idPratica);
                        return View(model);
                    }
                    else { return RedirectToAction("ErrorAuth", "Home"); }
                }
        ...
        public ActionResult UploadFile(int id, List<HttpPostedFileBase> files)
        {
            try
            {
                ...
                if (Request.Files != null && Request.Files.Count > 0 && Request.Files.Count <= 4)
                {
                    ...
                    if (ListFile.Count > 0)
                    {
                        if (!_help.LoadFileCredito(ListFile))
                        {
                ...
                List<Attachment> model = _help.GetFileCredito(id);
        ...
        public ActionResult DownloadFile(int idFile)
        {
            try
            {
                Attachment loadFile = _help.DownloadFileCredito(idFile);
        ...
        public ActionResult DeleateFile(int idPratica, int idFile)
        {
            try
            {
                ...
                if (_help.DelateFileCredito(idFile))
                {
                    model = _help.GetFileCredito(idPratica);
                    return PartialView("TableAllegati", model);
                }
